...
//RenderMinecart<EntityMinecartMobSpawner>
public class RenderOreCart<T extends EntityOreMinecart> extends Render<T>
{
	private static final ResourceLocation ORECART_TEXTURES = new ResourceLocation("harderores:textures/entity/orecart.png");
	/** instance of ModelMinecart for rendering */
	protected ModelOreCart modelMinecart = new ModelOreCart();

	public RenderOreCart(RenderManager renderManagerIn) {
	...
	public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks) {
		...
		double chuteYaw = Math.atan2((entity.posX - entity.lastTickPosX), (entity.posZ - entity.lastTickPosZ)) / Math.PI * 180;
		...
		//chuteYaw
		GlStateManager.rotate(-1*(180.0F - entityYaw), 0.0F, 1.0F, 0.0F);

		if((entity.posX - entity.lastTickPosX) == 0 && (entity.posZ - entity.lastTickPosZ) == 0) {
			chuteYaw = -entityYaw;
		}
		else if(entity.getDumpDir() == DumpDir.RIGHT) {
			chuteYaw += 90;
		}
		else {
			chuteYaw -= 90;
		}
		GlStateManager.rotate((float)chuteYaw, 0.0F, 1.0F, 0.0F);
		this.modelMinecart.renderChute(entity, -0.1f, 0.0625f);
		...
		float travelDirection = (float) Math.atan2(entity.motionX,entity.motionZ);
		travelDirection = entityYaw;//(float)(travelDirection/Math.PI * 180) + 90;
		
		//System.out.println((travelDirection) + " <> " + (entityYaw));
		GlStateManager.rotate(180.0F - travelDirection, 0.0F, 1.0F, 0.0F);

		float one = 1;
		if(travelDirection == Math.abs(entityYaw)) {
			//one = -1;
		}
		GlStateManager.rotate(-f3, 0.0F, 0.0F, 1.0F);
