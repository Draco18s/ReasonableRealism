...
public class ItemEntityOreCart extends Item
{
	private static final IBehaviorDispenseItem MINECART_DISPENSER_BEHAVIOR = new BehaviorDefaultDispenseItem()
	{
		...
		public ItemStack dispenseStack(IBlockSource source, ItemStack stack)
		{
			...
			EntityMinecart entityminecart = createCustomCart(world, d0, d1 + d3, d2, ((ItemEntityOreCart)stack.getItem()).minecartType);
	...
	public ItemEntityOreCart(EntityMinecart.Type typeIn)
	{
		this.maxStackSize = 1;
	...
	public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (!BlockRailBase.isRailBlock(iblockstate)) {
			...
			if (!worldIn.isRemote) {
				...
				EntityMinecart entityminecart = createCustomCart(worldIn, (double)pos.getX() + 0.5D, (double)pos.getY() + 0.0625D + d0, (double)pos.getZ() + 0.5D, this.minecartType);
	...
	protected static EntityMinecart createCustomCart(World worldIn, double x, double y, double z, EntityMinecart.Type typeIn) {
		/*if(typeIn.getId() <= 6) {
			return EntityMinecart.create(worldIn, (double)x, (double)y, (double)z, typeIn);
		}*/
		return new EntityOreMinecart(worldIn, x, y, z);
	}
}