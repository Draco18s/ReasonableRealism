...
public class BlockOreFlower2 extends BlockBush implements IBlockWithMapper, IShearable {
	...
	public boolean canSustainPlant(IBlockState state, IBlockAccess world, BlockPos pos, EnumFacing direction, IPlantable plantable) {
		...
		/*if(plantable.getPlant(world, pos).getBlock() == this) {
			if(state.getValue(Props.FLOWER_TYPE2) == EnumOreFlower2._5TANSY) {
				return true&f;
			}
			if(state.getValue(Props.FLOWER_TYPE2) == EnumOreFlower2._4FLAME_LILY) {
				return true&f;
			}
		}*/
	...
	public IBlockState getStateForPlacement(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer) {
		...
		EnumOreFlower2 thisType = state.getValue(Props.FLOWER_TYPE2);
		/*if(thisType == EnumOreFlower2._5TANSY || thisType == EnumOreFlower2._4FLAME_LILY) {
			IBlockState stateBelow = worldIn.getBlockState(pos.down());
			if(stateBelow.getBlock() == this && stateBelow.getValue(Props.FLOWER_TYPE2) == thisType) {
	...
	public IBlockState getStateFromMeta(int meta) {
		...
		return this.getDefaultState().withProperty(Props.FLOWER_TYPE2, EnumOreFlower2.values()[type]).withProperty(Props.FLOWER_STALK, stalk>0);
	}

	@Override
	public int getMetaFromState(IBlockState state) {
		int stalk = state.getValue(Props.FLOWER_STALK)?8:0;
		int type = state.getValue(Props.FLOWER_TYPE2).getOrdinal()%8;
		return stalk + type;
	}
	
	@Override
	public int damageDropped(IBlockState state) {
		return state.getValue(Props.FLOWER_TYPE2).getOrdinal();
	}
	
	@Override
	public void updateTick(World world, BlockPos pos, IBlockState state, Random rand) {
		super.updateTick(world, pos, state, rand);
		/*if(state.getValue(Props.FLOWER_TYPE2) == EnumOreFlower2._2HORSETAIL && rand.nextInt(100) == 0) {
	...
	public StateMapperBase getStateMapper() {
		return new StateMapperFlowers(Props.FLOWER_TYPE2);
	}
    ...
    public List<ItemStack> onSheared(ItemStack item, IBlockAccess world, BlockPos pos, int fortune) {
        ...
        ret.add(new ItemStack(this, 1, world.getBlockState(pos).getValue(Props.FLOWER_TYPE2).getOrdinal()));
        return ret;
    }
}
