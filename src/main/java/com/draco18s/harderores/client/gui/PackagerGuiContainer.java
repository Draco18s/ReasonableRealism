...
public class PackagerGuiContainer extends ContainerScreen<PackagerContainer> {
	PackagerTileEntity tileEntity;
	Container container;
	private static ResourceLocation SIFTER_GUI_TEXTURE;

	public PackagerGuiContainer(PackagerContainer screenContainer, PlayerInventory inv, ITextComponent containerName) {
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
		...
		if(tileEntity != null) {
			int progress = Math.round(24 * (100 - tileEntity.getTime()) / 100.0f);
